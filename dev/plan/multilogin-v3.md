...
| 7 | implementing | Started 2026-08-01T10:26:24+08:00 at baseline `c9ac3ad`. Main-agent CodeGraph/source investigation confirmed browser password exposure through full UCI loads, direct log shell/file access, generic init/service grants, prefix-destructive network cleanup, missing ownership/recovery state, and obsolete navigation. `docs/v3/contracts.md` now freezes exact configuration/account/instance/service/diagnostic/log/network RPCs, strict types/bounds, password write-only behavior, separate service apply, exact ownership state, journal reducer, hidden route aliases, and final ACL split. Three review rounds resolved token/creation rules, JSON types, credential/IP/MAC redaction, reducer precedence, deterministic owned UCI templates, and absent/invalid state handling. | Read-only Terra `/root/phase6_review` final rereview: `PASS`; backend/UI/test implementation may start on disjoint ownership. | D-010/D-012/D-013 apply. No UCI, service, log, network, reboot, browser, or OpenWrt behavior will be simulated; automated checks are compile/static plus pure request/ownership/reducer/serialization logic. |
