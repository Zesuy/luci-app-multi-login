...
| 9 regression follow-up | accepted (automated) | 2026-08-02; device evidence after r3 showed that preserving `JSON_PREFIX` and pre-source `JSON_UNSET` was still insufficient: `json_load '{}'` emits a second stock `json_init`, whose cleanup unsets `JSON_UNSET` before the first `json_add_*`. Added a nounset-safe `json_init` override after sourcing JSHN in both fixed RPC helpers, bumped the package revision to `r4`, and retained the live-root, best-effort rpcd restart. Required gate passed `19/19` with zero optional skips; local BusyBox ash/JSHN probe covered generated-init plus json_add calls; clean 24.10 SDK IPK compile passed with SHA-256 `ede14a2f7a007a22e638cfa0619cb65e1417140985bd7d10ab34145bb72c89a4`. | Independent read-only reviewer `/root/rpc_jshn_review`: `PASS`; verified SDK JSHN `-r` generated init behavior, stock state setup, second-init recovery, BusyBox/bash nounset probes, API/schema and secret-boundary preservation, and r4 metadata consistency. | D-020 extended. The new r4 IPK must receive real-device Phase 9 installation/RPC verification before RC acceptance; no portal, network, or device mutation was automated. |
