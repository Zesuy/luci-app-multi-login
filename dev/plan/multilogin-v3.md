...
| 9 regression follow-up (r8) | accepted (automated) | 2026-08-02; baseline `b29b9df`; accepted content commits `cf0bd26` and `87ae27f`; main agent. Playwright on disposable OpenWrt 23.05.5 captured browser `{}` requests and rpcd helper stdin `{"ubus_rpc_session":"<session>"}`; r7 returned `invalid request fields`/`invalid request`, while direct `ubus call` passed. Both backends now type-check and filter only that metadata field; all other unknown fields remain exact-rejected and filtered config fields are canonically sorted. Added `tests/qemu-playwright-smoke.mjs` as an explicitly optional disposable-QEMU diagnostic (not in the bounded default runner) and Phase 7 static regressions. Before the fix: overview and scripts pages reproduced the user-visible errors. After the fix: Playwright overview and scripts pages passed the request-boundary check; overview rendered data and script_info returned `ok` (fresh QEMU still reports expected `recovery_required` because no active script was initialized by the manual package extraction). Negative QEMU probes rejected wrong-type session metadata and extra fields with valid JSON envelopes. Local `CI=0 MULTILOGIN_REQUIRE_TOOLING=0 ./tests/run.sh` passed `16` checks with 4 optional tooling skips; focused Phase 5 `10/10`, Phase 7 `14/14`, Phase 8 `7/7`, version matrix, shell syntax, `git diff --check`, and Playwright smoke passed. Package revision is now `r8` so the fixed artifact cannot be mistaken for r7. Cloud CI run `30752936857` passed offline tests, both SDK JSHN contracts, 24.10.8 IPK compilation/inspection, and 25.12.5 APK compilation/inspection; artifact SHA-256 values are `8c048ff84a52c6fff3e200f0fc8ffb0f2dd72f5d72a4c2cbd6908c9bab964fa6` (IPK) and `9d6c68ea1158a51e48215f28655d3cc6160bb884f39e35553b3d474aa6118f5a` (APK). | Independent read-only `/root/r7_review`: `PASS`; confirmed metadata type checks, exact-schema preservation, filtered-field sorting, and no secret/portal-action changes. No portal login/logout or real-device action occurred. | D-023 added. Package installation and Phase 9 device retry remain manual-authority. |
