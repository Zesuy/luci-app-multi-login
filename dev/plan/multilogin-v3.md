...
| 5 | implementing | Started 2026-08-01 at baseline `9b33399`; main-agent CodeGraph/source investigation found no current script RPC family and confirmed the LuCI page still directly reads/writes/chmods legacy scripts. `docs/v3/contracts.md` now freezes exact Phase 5 parameters/data/error codes, summary/state/journal schemas, no-op/conflict behavior, transaction/LKG/recovery rules, fixed Raw policy, Custom draft bounds, execution confirmations, and source exposure. | First Terra contract review returned `BLOCK` for recovery, validation ordering, source/password, and HTTP classification gaps; corrections were independently rereviewed read-only by Terra `/root/phase4_prod_fix`, which returned `PASS`. | D-010 applies: no network/local-HTTP/OpenWrt/filesystem activation simulation. Real download execution, activation, rollback, self-test/status, locking/atomicity, recovery, and device behavior remain mandatory Phase 9 checks. |
