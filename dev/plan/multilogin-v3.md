...
| 8 | investigating | Started 2026-08-01T13:08:00+08:00 at baseline `bca5117`. Initial source inventory found one pinned offline CI workflow, no SDK/release workflow, legacy tags `1.1.0-r1`, `2.0.2-r2`, and `alpha`, package metadata still `2.2.0-4`, and script metadata already `3.0.0-rc.1`/API 3. Phase 8 will use x86_64 SDKs as architecture-independent package compile witnesses for supported OpenWrt 23.05 and 24.10, with read-only archive inspection rather than installation. | CI/release/test implementation and independent review pending. | Push/tag/release/privileged dispatch remain manual. Required SDK acquisition/execution failure after bounded retries blocks this phase. |
