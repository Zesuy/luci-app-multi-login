...
| 输入 | 固定身份 | 当前离线证据 |
| --- | --- | --- |
| v3 / OpenWrt 24.10 | `luci-app-multilogin_3.0.0-rc.1-r16_all.ipk` | CI artifact checksum required before device acceptance |
| v3 / OpenWrt 25.12 | `luci-app-multilogin-3.0.0_rc1-r16.apk` | CI artifact checksum required before device acceptance |
...
| `PKG-02` | 24.10 全新安装 | 与 `PKG-01` 相同，并确认 `3.0.0-rc.1-r16` 控制版本。 |
| `PKG-02A` | 25.12 APK 全新安装 | 使用真实 apk-tools 和真实设备；与 `PKG-01` 相同，确认 `3.0.0_rc1-r16`、`noarch`、依赖、conffile、lifecycle hooks 与卸载清理。未通过时不得发布 APK。 |
