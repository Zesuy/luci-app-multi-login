...
- Latest committed fix: `240f7f2 Fix LuCI boolean disabled attributes`
- Target product: MultiLogin v3, `v3.0.0-rc.1`
- Current package revision: `r9`
...
LuCI 23.05 `E()` serializes non-null attributes with `setAttribute()`. Passing `disabled: false` therefore created `disabled="false"`, disabling every button. The five MultiLogin views now omit the attribute when idle and emit it only for busy/recovery states. The package revision was bumped from r8 to r9.

## Validation evidence

The bounded local runner passed 16 checks; four optional tools were unavailable locally. JavaScript syntax, focused Phase 6/7 checks, and `git diff --check` passed. Cloud compilation and device installation of r9 are still pending.
...
6. Do not push, release, install packages, mutate a real device, or perform portal login/logout without explicit authorization.
