...
function harness(name, options = {}) {
  ...
  const extendedInstances = instances.map((x) => `multilogin.${x.section}=instance`).join(' ');
  const ordinaryInstances = instances.map((x, index) => `multilogin.${x.anonymous ? `@instance[${index}]` : x.section}=instance`).join(' ');
  uci.push(`  *"-X show multilogin") ${options.noInstances ? ':' : `printf '%s\\n' ${extendedInstances}`} ;;`);
...
assert(fs.readFileSync(controller, 'utf8').includes('uci -q -X show multilogin'), 'controller does not enumerate generated UCI section names');
...
{ const h=harness('anonymous-instance',{loops:1,instances:[{section:'cfg_anon_instance',iface:'wan1',account:'a1',username:'u1',anonymous:true}]}); const r=run(h); assert(r.status===0&&calls(h)===1,'anonymous enabled instance was skipped'); pass('legacy anonymous instance enumeration'); }
