...
return view.extend({
    ...
    render: function (initial) {
        ...
        function draw() {
            ...
            root.replaceChildren.apply(root, compact([E('h2', {}, _('诊断')), E('p', { class: 'cbi-map-descr' }, _('显示固定 MultiLogin 日志的有界、经过服务器端脱敏的内容。不会读取任意文件或显示原始命令输出。')),
                state.feedback ? E('div', { class: state.error ? 'alert-message' : 'alert-message notice', role: state.error ? 'alert' : 'status' }, compact([E('p', {}, state.feedback), state.error ? button(_('重试'), function () { refresh(); }, state.busy) : null])) : null,
