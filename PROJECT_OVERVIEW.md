...
脚本状态与网络所有权状态均为 root-only 文件，并通过代次、锁、原子写入和事务日志保护。脚本的自定义草稿是管理员输入的 root 级代码，不应包含任何秘密；活动、工厂、候选和备份脚本内容均不由浏览器直接读取。管理员可明确请求把哈希匹配的托管活动脚本复制成 mode-0600 隔离草稿，再通过唯一的草稿读取边界进行编辑；Custom 或未知活动脚本不能通过该入口复制。
